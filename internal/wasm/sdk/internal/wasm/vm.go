...
type VM struct {
	store                *wasmer.Store
	instance             *wasmer.Instance // Pointer to avoid unintented destruction (triggering finalizers within).
	policy               []byte
	data                 []byte
...
func opaFunctions(vm *VM, store *wasmer.Store, imports *wasmer.ImportObject) {

	opaAbort := wasmer.NewFunction(
		store,
	...
	opaBuiltin0 := wasmer.NewFunction(
		store,
		wasmer.NewFunctionType(wasmer.NewValueTypes(wasmer.I32, wasmer.I32), wasmer.NewValueTypes(wasmer.I32)),
		func(args []wasmer.Value) ([]wasmer.Value, error) {
			return []wasmer.Value{wasmer.NewI32(vm.Builtin(args[0].I32(), args[1].I32()))}, nil
		},
	)

	opaBuiltin1 := wasmer.NewFunction(
		store,
		wasmer.NewFunctionType(wasmer.NewValueTypes(wasmer.I32, wasmer.I32, wasmer.I32), wasmer.NewValueTypes(wasmer.I32)),
		func(args []wasmer.Value) ([]wasmer.Value, error) {
			return []wasmer.Value{wasmer.NewI32(vm.Builtin(args[0].I32(), args[1].I32(), args[2].I32()))}, nil
		},
	)

	opaBuiltin2 := wasmer.NewFunction(
		store,
		wasmer.NewFunctionType(wasmer.NewValueTypes(wasmer.I32, wasmer.I32, wasmer.I32, wasmer.I32), wasmer.NewValueTypes(wasmer.I32)),
		func(args []wasmer.Value) ([]wasmer.Value, error) {
			return []wasmer.Value{wasmer.NewI32(vm.Builtin(args[0].I32(), args[1].I32(), args[2].I32(), args[3].I32()))}, nil
		},
	)

	opaBuiltin3 := wasmer.NewFunction(
		store,
		...
		func(args []wasmer.Value) ([]wasmer.Value, error) {
			return []wasmer.Value{wasmer.NewI32(vm.Builtin(args[0].I32(), args[1].I32(), args[2].I32(), args[3].I32(), args[4].I32()))}, nil
		},
	)

	opaBuiltin4 := wasmer.NewFunction(
		store,
		...
		func(args []wasmer.Value) ([]wasmer.Value, error) {
			return []wasmer.Value{wasmer.NewI32(vm.Builtin(args[0].I32(), args[1].I32(), args[2].I32(), args[3].I32(), args[4].I32(), args[5].I32()))}, nil
		},
	)

	imports.Register("env", map[string]wasmer.IntoExtern{
		"opa_abort":    opaAbort,
		"opa_builtin0": opaBuiltin0,
		"opa_builtin1": opaBuiltin1,
		"opa_builtin2": opaBuiltin2,
		"opa_builtin3": opaBuiltin3,
		"opa_builtin4": opaBuiltin4,
	})
}
...
func newVM(opts vmOpts) (*VM, error) {
	...
	limits, err := wasmer.NewLimits(opts.memoryMin, opts.memoryMax)
	if err != nil {
		return nil, err
	}
	...
	opaFunctions(v, store, imports)

	module, err := wasmer.NewModule(store, opts.policy)
	if err != nil {
		return nil, err
	}

	i, err := wasmer.NewInstance(module, imports)
	if err != nil {
		return nil, err
	}
	...
	if opts.parsedData != nil {
		if uint32(memory.DataSize())-uint32(v.baseHeapPtr) < uint32(len(opts.parsedData)) {
			...
			memory.Grow(wasmer.Pages(Pages(delta))) // TODO: Check return value?
		}
		mem := memory.Data()
	...
	fn, err := i.Exports.GetFunction("builtins")
	if err != nil {
		return nil, err
	}
	...
	fn, err = i.Exports.GetFunction("entrypoints")
	if err != nil {
		return nil, err
	}
...
func (i *VM) SetPolicyData(opts vmOpts) error {
	...
	if opts.parsedData != nil {
		if uint32(i.memory.DataSize())-uint32(i.baseHeapPtr) < uint32(len(opts.parsedData)) {
			...
			i.memory.Grow(wasmer.Pages(Pages(delta))) // TODO: Check return value?
		}
		mem := i.memory.Data()
...
func call(vm *VM, name string, args ...int32) (int32, error) {
	...
	fn, err := vm.instance.Exports.GetFunction(name)
	if err != nil {
		return 0, err
	}
