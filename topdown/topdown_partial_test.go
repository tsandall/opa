...
func TestTopDownPartialEval(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			noinline: []string{`data.test.q`},
		},
		{
			note:  "noinline: complete doc with suffix",
			query: "data.test.p = true",
			modules: []string{`
				package test
				p { s; q[x] }
		...
		{
			note:  "noinline: partial doc with suffix",
			query: "data.test.p = true",
			modules: []string{`
				package test
				p { y = 0; q[x][y]; r }
	...
	for _, tc := range tests {
		...
		prepareTest(ctx, t, params, func(ctx context.Context, t *testing.T, f fixture) {
			...
			noinline := make([]ast.Ref, len(tc.noinline))
			for i, s := range tc.noinline {
				noinline[i] = ast.MustParseRef(s)
			}
