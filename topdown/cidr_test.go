...
func TestNetCIDRContainsMatches(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			// NOTE(tsandall): shouldn't this be a built-in error?
			expected: &Error{Code: TypeErr, Message: "net.cidr_contains_matches: operand 1 must be string but got number"},
		},
		{
		...
		},
		{
			...
			expected: &Error{Code: BuiltinErr, Message: `net.cidr_contains_matches: operand 2: set element must be string or non-empty array`},
		},
		{
		...
		{
			note:     "objects",
			rules:    []string{`p = x { x := net.cidr_contains_matches({"k1": "1.1.1.1/24"}, "1.1.1.128") }`},
			expected: `[["k1", "1.1.1.128"]]`,
		},
	}
